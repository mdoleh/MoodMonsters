...
public class GUIManager : MonoBehaviour 
{
    ...
    void PauseLevel()
    {
        ...
        pauseMovement = (pauseButton.transform.localPosition.x - pauseMenu.transform.localPosition.x) - 0.85f;
        
        //Hide the play button and bring the pause menu to middle position
        StartCoroutine(MoveMenuElementBy(playButton, 1.5f, 0.4f, 0));
        StartCoroutine(MoveMenuElementBy(pauseMenu, pauseMovement, 0.2f, 0));

        //Move the pause menu to the extended position with the pause button
        StartCoroutine(MoveMenuElementBy(pauseMenu, 0.65f, 0.2f, 0.2f));
        StartCoroutine(MoveMenuElementBy(pauseButton, 0.65f, 0.2f, 0.2f));
    ...
    void UnpauseLevel()
    {
        ...
        StartCoroutine(MoveMenuElementBy(pauseMenu, -0.65f, 0.2f, 0));
        StartCoroutine(MoveMenuElementBy(pauseButton, -0.65f, 0.2f, 0));

        //Show the play button and hide the pause menu
        StartCoroutine(MoveMenuElementBy(playButton, -1.5f, 0.4f, 0.2f));
        StartCoroutine(MoveMenuElementBy(pauseMenu, -pauseMovement, 0.2f, 0.2f));
